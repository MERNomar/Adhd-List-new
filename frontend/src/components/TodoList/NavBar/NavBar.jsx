...
export default function NavBar() {
    ...
    const githubLink = "https://github.com/MERNomar/TodoList-react"
  ...
  return (
    <nav className=" w-full justify-between  lg:w-[calc(100%-18rem)] flex items-center h-[60px] border-b-[1px] fixed top-0 z-50   NavBar m-0 right-0 border-[#ffffff3d] bg-[#23272f]  stroke-neutral-950 rounded-sm  ">
       <div>{isSizeOk ? <button className='ml-5  nav-item' onClick={() => {setIsSideNavOpen(!isSideNavOpen)}}><Dehaze className='icon-button'/></button>: null}</div>
       <div className='flex'>
      <button  onClick={() => setLogout(null)} className=' nav-item'><Person className='icon-button' /></button>
       <button onClick={() => setDarkMode()}  className=' nav-item'><Brightness4 className='icon-button'/></button>
      <a target='_blank' href={githubLink} className='mr-7 nav-item'><GitHub className='icon-button'/></a>
      </div>
